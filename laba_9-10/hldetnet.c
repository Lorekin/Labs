    ...
    for (i = 0; i < lines; i++) {
        ...
        while (sent < msgsize) {
            switch (poll(&pfd, 1, -1)) {
                ...
                case 1:
                    nread = write(csock, msg + sent, msgsize - sent);
                    if (nread == -1)
                        err(1, "write");
                    else
                        sent += nread;
...
int main(int argc, char **argv) {
    ...
    if (argc < 0) usage("invalid mode, should be either server or client");
    memset(&ss, 0, sizeof(struct sockaddr_storage));
    if (strcmp(argv[1], "-p") != 0) usage("invalid flag, try to use \"-p\" instead");
    size_t socket_path_length = strlen(argv[2]);
    char *socket_addr = malloc(socket_path_length);
    strlcpy(socket_addr, argv[2], socket_path_length); // copying path to string

    struct sockaddr_un *sun = (struct sockaddr_un *) &ss;
    sun->sun_family = AF_UNIX;
    if (strlcpy(sun->sun_path, argv[3], sizeof(sun->sun_path)) >= sizeof(sun->sun_path)) usage("UNIX socket path is too long");
    ...
    if (strcmp(argv[4], "inet") == 0 || strcmp(argv[4], "inet6") == 0) {
        int	port, rv;

        ss.ss_family = ((argv[2][4] == '\0') ? AF_INET : AF_INET6);
        port = atoi(argv[3]);
        if (port <= 0 || port > 65535) errx(1, "invalid port: %s", argv[3]);
        if (argc > 4) {
            ...
            if (!rv) errx(1, "invalid network address: %s", argv[4]);
        } else if (servermode) {
            /*
